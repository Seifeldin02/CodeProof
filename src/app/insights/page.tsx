...
import { CheckCircleIcon, ClockIcon, SearchCodeIcon, TrendingUpIcon, UsersIcon } from "@/components/ui/icons";
...
export default async function HiringInsightsPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader eyebrow={t("Hiring operations")} title={t("Hiring Insights")} description={t("Explainable hiring metrics connected to the same repository evidence recruiters review in each candidate dossier.")} badge={activeCandidates.length > 0 ? <Badge tone={realCandidates.length ? "positive" : "warning"}>{realCandidates.length ? t(realCandidates.length === 1 ? "{count} real candidate" : "{count} real candidates", { count: realCandidates.length }) : t("Synthetic demo analytics")}</Badge> : undefined} />
      {candidates.length === 0 ? <Card bodyClassName="!p-0"><EmptyState icon={SearchCodeIcon} title={t("Insights need candidate records")} description={t("Analyze candidates first; this dashboard remains empty instead of inventing metrics.")} actionLabel={t("Analyze a candidate")} actionHref="/analyze" /></Card> : <>

        ...
        <tbody>
          {data.map((s) => (
            <tr key={s.source} className="border-b border-slate-50 last:border-0">
              <td className="py-2.5 font-medium text-slate-700">{s.source}</td>
              <td className="tnum py-2.5 text-right text-slate-600">{s.applicants}</td>
              <td className="tnum py-2.5 text-right font-medium text-slate-800">{s.hireRate}%</td>
              <td className="tnum py-2.5 text-right text-slate-600">
                {s.avgTimeToHireDays === null ? '—' : `${s.avgTimeToHireDays}d`}
              </td>

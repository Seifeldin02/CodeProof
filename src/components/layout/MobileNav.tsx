...
export default function MobileNav() {
  ...
  const { t } = useI18n();
  return (
    <nav className="no-print fixed inset-x-0 bottom-0 z-40 grid grid-cols-5 border-t border-slate-200 bg-white/95 px-1 pb-[max(.35rem,env(safe-area-inset-bottom))] pt-1.5 shadow-[0_-10px_30px_rgba(15,23,42,.08)] backdrop-blur-xl lg:hidden" aria-label={t("Recruiting workspace")}>
      {navItems.map((item) => {

...
const PIPELINE_STEPS = [
  ["CV intake", "Extract links"],
  ["Project scope", "Recruiter confirms"],
  ["Source scan", "Bounded archives"],
  ["Evidence dossier", "Persist report"],
] as const;
...
function PipelineMap({ discovery, busy }: { discovery: CvDiscovery | null; busy: "discover" | "analyze" | null }) {
  ...
  return <div className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white/90 p-4 shadow-card"><div className="grid gap-2 sm:grid-cols-4">{PIPELINE_STEPS.map(([title, copy], index) => { const complete = index < activeIndex; const active = index === activeIndex; return <div key={title} className={`relative rounded-xl border p-3 transition-all duration-500 ${active ? "border-brand-300 bg-brand-50 shadow-sm" : complete ? "border-emerald-200 bg-emerald-50/60" : "border-slate-100 bg-slate-50/70"}`}><div className="flex items-center gap-2"><span className={`grid h-6 w-6 place-items-center rounded-full font-mono text-[9px] font-semibold ${active ? "evidence-node bg-brand-700 text-white" : complete ? "bg-emerald-600 text-white" : "bg-slate-200 text-slate-500"}`}>{complete ? "✓" : `0${index + 1}`}</span><strong className="text-xs text-slate-800">{title}</strong></div><p className="mt-2 pl-8 text-[10px] text-slate-400">{active && busy ? busy === "discover" ? "Reading candidate material…" : "Inspection in progress…" : copy}</p>{index < PIPELINE_STEPS.length - 1 && <span className="absolute -right-2 top-1/2 z-10 hidden h-px w-2 bg-slate-300 sm:block" />}</div>; })}</div></div>;
}
    ...
    <div className="space-y-5">
      ...
      <section className="surface-card rounded-2xl border border-slate-200/80 bg-white/95 p-5 shadow-card sm:p-7">
        ...
        {!discovery ? <div className="mt-6 space-y-5">
          ...
          <button type="button" onClick={() => runDiscovery()} disabled={busy !== null || (!resumeFile && !resumeText.trim())} className="group flex w-full items-center justify-between rounded-xl bg-slate-950 px-5 py-3.5 text-sm font-semibold text-white transition-all hover:-translate-y-0.5 hover:bg-brand-700 disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:translate-y-0"><span className="flex items-center gap-2">{busy === "discover" && <i className="h-3 w-3 animate-spin rounded-full border border-white/30 border-t-white" />}{busy === "discover" ? "Reading CV…" : "Detect candidate projects"}</span><span className="transition-transform group-hover:translate-x-1">→</span></button>
          ...
          {discovery.profiles.length > 0 && <div className="rounded-xl border border-amber-200 bg-amber-50 p-4"><div className="flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-amber-500" /><strong className="text-sm text-amber-900">GitHub profile detected</strong></div><p className="mt-2 text-xs leading-5 text-amber-800">{discovery.profiles.map((profile) => profile.url).join(", ")}. Free archive scanning cannot list profile projects, so paste the repository URLs you want to inspect.</p></div>}
          <div><div className="flex items-center justify-between"><h3 className="text-sm font-semibold text-slate-900">Selected repositories</h3><span className="text-xs text-slate-400">{selected.size}/3 selected</span></div><div className="mt-3 grid gap-2 sm:grid-cols-2">{repositories.map((repository) => <button type="button" key={repository.url} onClick={() => toggleRepository(repository.url)} className={`interactive-card flex w-full items-center gap-3 rounded-xl border p-4 text-left transition ${selected.has(repository.url) ? "border-brand-300 bg-brand-50 ring-2 ring-brand-100" : "border-slate-200 bg-white hover:border-brand-200"}`}><span className={`grid h-6 w-6 place-items-center rounded-md border text-xs ${selected.has(repository.url) ? "border-brand-700 bg-brand-700 text-white" : "border-slate-300 text-slate-400"}`}>{selected.has(repository.url) ? "✓" : "+"}</span><span className="min-w-0"><strong className="block truncate text-sm text-slate-900">{repository.owner}/{repository.repository}</strong><small className="text-slate-400">Public source archive</small></span></button>)}</div>
          ...
          <div className="flex gap-3"><button type="button" onClick={() => { setDiscovery(null); setError(null); }} disabled={busy !== null} className="rounded-xl border border-slate-200 px-5 py-3 text-sm font-semibold text-slate-600 transition hover:bg-slate-50">Back</button><button type="button" onClick={analyze} disabled={busy !== null || selected.size === 0} className="group flex flex-1 items-center justify-between rounded-xl bg-slate-950 px-5 py-3 text-sm font-semibold text-white transition-all hover:-translate-y-0.5 hover:bg-brand-700 disabled:opacity-50 disabled:hover:translate-y-0"><span className="flex items-center gap-2">{busy === "analyze" && <i className="h-3 w-3 animate-spin rounded-full border border-white/30 border-t-white" />}{busy === "analyze" ? `Inspecting ${selected.size} source archive${selected.size === 1 ? "" : "s"}…` : "Build candidate evidence report"}</span><span className="transition-transform group-hover:translate-x-1">→</span></button></div>
        </div>}
      ...
      <aside className="space-y-4">
        <div className="hero-grid rounded-2xl bg-slate-950 p-6 text-white"><p className="text-[10px] font-semibold uppercase tracking-[.18em] text-lime-300">What CodeProof inspects</p><ul className="mt-5 space-y-4 text-sm">{[["Languages and tools","Extensions, manifests and real imports."],["Architecture patterns","Routes, services, state, auth and persistence boundaries."],["Implementation depth","Repeated meaningful usage with exact file citations."],["Quality signals","Tests, error handling, observability and deployment evidence."]].map(([title, copy], index) => <li key={title} className="flex gap-3 border-b border-white/10 pb-4 last:border-0 last:pb-0"><span className="font-mono text-[10px] text-lime-300">0{index + 1}</span><span><strong className="block">{title}</strong><span className="mt-1 block text-xs leading-5 text-slate-400">{copy}</span></span></li>)}</ul></div>
